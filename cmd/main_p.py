...
from chain_simulator.blockchain.blockchain_p import BlockchainStates, BlockchainMProc
...
def schedule(cmd_args: dict):
    ...
    bcstates_manager = BlockchainStatesManager(address="temp/BlockchainStatesManager")
    ...
    sync_mempool = bcstates_manager.SyncMemPool(maxsize=cmd_args.mempool_size)
    ...
    proc_bc = multiprocessing.Process(target=start_bc, args=(config, blockchain_states))
    proc_bc.start()
