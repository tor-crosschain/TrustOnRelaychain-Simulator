...
def build_and_start_relayers(hostsmap: Dict[str, Dict[int, str]]):
    ...
    # Only take the first one, temporarily do not consider the case of multiple gateways, but the structure of hostsmap supports multiple gateways
...
async def test_aor_async(setting: TestSetting):
    ...
    The parachains interact with the relay chain (requires synchronization of the block header and cross-chain transactions);
