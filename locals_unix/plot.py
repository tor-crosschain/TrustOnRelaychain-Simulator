...
datapath = "./locals_unix/output/indicates"

output_dir = "./locals_unix/plot"

color = {"ToR": "#3A5985", "AoR": "#F7A049", "NoR": "#CF4037"}
# color={"ToR":"#85C5F4","AoR":"#1E2F4D","NoR":"#CF4037"}
color_bar = {"ToR": "#C8EBF6", "AoR": "#EDCDCE", "ToRLine": "#3A5985", "AoRLine": "#F7A049"}
color_box="gainsboro"
...
class Data:
    ...
    def average(self):
        """
        The read() function puts multiple experimental results of the same indicator into an array (although the data storage method in locals/evaluate.py does not support multiple).
        The average() function calculates the average value of the data for the same indicator.
        """
    ...
    def read_aor(self, filepath: str, chain_num: int, xtxratio: float):
        ...
        for paral_chain, paral_loads in workloads.items():
            ...
            paral_loads_max.append(max(paral_loads))
            paral_bns_avg.append(len(paral_loads))
            # start_index = len(paral_loads) // 2 - 3
            # end_index = len(paral_loads) // 2 + 4
            # print(start_index)
            # print(end_index)
            # paral_loads_avg.append(sum(paral_loads[start_index:end_index]) / (end_index - start_index + 1))
            # paral_loads_max.append(max(paral_loads))
    ...
    def read_nor(self, filepath: str, chain_num: int, xtxratio: float):
        ...
        # print(len(paral_loads_avg))
        # paral_load_avg = sum(paral_loads_avg) / len(paral_loads_avg)
        # paral_load_max = max(paral_loads_max)
        paral_bn_avg = sum(paral_bns_avg) / len(paral_bns_avg)

        # if len(paral_loads_avg) == 0:
        #     print(filepath)
        #     paral_load_avg = 1.6
        #     paral_load_max = 3.6
        paral_load_avg = sum(paral_loads_avg) / len(paral_loads_avg)
...
def paral_workload_change():
    datapath_load = "./locals_unix/output/workloads"
    filenameToR = [
        "ToR-30-500-20.json",
        "ToR-70-500-20.json",
        "ToR-100-500-20.json",
...
def essay_paral_workload_change():
    ...
    filenameToR = [
        "ToR-30-1000-20.json",
        "ToR-70-1000-20.json",
        "ToR-100-1000-20.json",
        "ToR-140-1000-20.json",
    ]
    filenameAoR = [
        "AoR-30-1000-20.json",
        "AoR-70-1000-20.json",
        "AoR-100-1000-20.json",
        "AoR-140-1000-20.json",
    ]
    filenameNoR = [
        "NoR-30-1000-20.json",
        "NoR-70-1000-20.json",
        "NoR-100-1000-20.json",
        "NoR-140-1000-20.json",
    ]
    ...
    for i in range(0, 4, 1):
        ...
        if index in [1,2]:
            ...
            yticklabels = ['{}k'.format(ytick) for ytick in [0.5,1,1.5,2,2.5]]
            ...
            yticklabels = ['{}k'.format(ytick) for ytick in [1,2,3,4]]
            ax.set_yticks(yticks)
            ax.set_yticklabels(yticklabels)
            ax.set_ylabel("workload")

        ax.plot(xToR, yToR, label="ToR", marker="D",  linewidth=3,mew=5,ms=3,color=color["ToR"])
        ax.plot(xAoR, yAoR, label="AoR", marker="s",  linewidth=3,mew=5,ms=3,color=color["AoR"])
        ax.plot(xNoR, yNoR, label="NoR", marker="^",  linewidth=3,mew=5, ms=3,color=color["NoR"])

        ax.legend(loc="upper right", bbox_to_anchor=(1, 1), fontsize="medium")

        if parachainnum[i] in [10]:
            fig.savefig(output_path("output_essay_paralchain_workload_change" + str(index) + ".jpg"))
            fig.clf()
            continue

        axins = ax.inset_axes((0.53, 0.16, 0.4, 0.3))
        ...
        axins.set_xlim(xlim0, xlim1)
        axins.set_ylim(ylim0, ylim1)
        axins.set_yticks([i for i in range(0,300,100)])
        ...
        axins.fill(axins_sx, axins_sy, color=color_box)
...
def essay_total_latency():
    ...
    for pn in [10, 60, 120, 180]:
        ...
        ax.plot(x, y, label="ToR", marker="D", linewidth=3,mew=4,ms=8,color=color["ToR"], linestyle='-.')
        ax.plot(x1, y1, label="AoR", marker="s", linewidth=3,mew=4,ms=8,color=color["AoR"], linestyle='-.')
        ax.plot(x2, y2, label="NoR", marker="^", linewidth=3,mew=4,ms=8,color=color["NoR"], linestyle='-.')
        ax.legend()
        if pn == 10:
            xToR = x
            yToR = y
            ...
            axins.set_xlim(xlim0, xlim1)
            axins.set_ylim(ylim0, ylim1)
            axins.set_yticks([i for i in range(20, 65, 15)])
...
def essay_tps():
    ...
    for pn in [10, 60, 120, 180]:
        ...
        if index == 1:
            ...
            ax.set_ylim(ymin=0, ymax=2050)
            ax.set_yticks([i for i in range(0, 2400, 400)])
        elif index == 4:
            ax.set_ylim(ymin=0, ymax=2550)
            ax.set_yticks([i for i in range(0,3000,500)])
        ax.set_title(f"pn={pn}")
        ax.set_xlabel("ratio")
        ax.set_ylabel("TPS")
        # ax.set_ylim(ymin=0, ymax=1200)
...
def essay_inter_load_avg():
    ...
    for pn in [60, 180]:
        ...
        if index == 2:
            ...
            ax.set_yticks([i for i in range(0,24,4)])
        # ax.set_ylim(ymin=0, ymax=160)
        ax.plot(x, y, label="ToR-avg", marker="D", linewidth=3, mew=5, ms=8,color = color_bar["ToRLine"],linestyle='-.')
        ax.plot(x1, y1, label="AoR-avg", marker="s", linewidth=3, mew=5, ms=8, color=color_bar["AoRLine"],linestyle='-.')
        ax.bar(
            [i - width / 2 for i in xx],
            yy,
            width=width,
    ...
    for ratio in [0.2, 0.8]:
        ...
        # ax.set_ylim(ymin=0, ymax=160)
        ax.plot(x_step, y, label="ToR-avg", marker="D", linewidth=3, mew=5, ms=8,color = color_bar["ToRLine"],linestyle='-.')
        ax.plot(x1_step, y1, label="AoR-avg", marker="s", linewidth=3, mew=5, ms=8,color = color_bar["AoRLine"],linestyle='-.')
        # ax.legend()
        ax.bar(
...
def essay_scalability():
    ...
    for ratio in [0.8]:
        ...
        ax.set_ylabel("TPS")
        # ax.set_ylim(ymin=10, ymax=500)
        ax.plot(x, y, label="ToR", marker="D",linewidth=3,mew=4,ms=8,color=color["ToR"])
        ax.plot(x1, y1, label="AoR", marker="s",linewidth=3,mew=4,ms=8,color=color["AoR"])
        ax.plot(x2, y2, label="NoR", marker="^",linewidth=3,mew=4,ms=8,color=color["NoR"])
        ax.legend()
        # ax.grid(linestyle='--')
        fig.tight_layout()
    fig.savefig(output_path("output_essay_scalability1.jpg"))
    fig.clf()
    ...
    # lines, labels = fig.axes[-1].get_legend_handles_labels()
    # fig.legend(lines, labels, loc="upper left")
    fig.savefig(output_path("output_essay_scalability2.jpg"))
...
def essay_inter_workload_ratio():
    ...
    for pn, value in sorted(data.pn_rt_tor.items(), key=lambda x: x[0]):
        ...
        a, b = x, [1 - p / p1 for (p, p1) in zip(y, y1)]
        bavg = sum(b) / len(b)
        diff2 = (sum((x - bavg) ** 2 for x in b) / len(b)) ** (1 / 2)  # 标准差
        print(f"diff2 of pn({pn}): {diff2}")
        # a, b = x, [1-math.log(p,2)/math.log(p1, 2) for (p, p1) in zip(y, y1)]
        maxv = max(maxv, max(b))
        minv = min(minv, min(b))
