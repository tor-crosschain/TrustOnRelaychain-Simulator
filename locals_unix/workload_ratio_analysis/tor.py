...
async def build_and_start_relayers(hostsmap: Dict[str, Dict[int, str]]):
    ...
    # Start the gateway of the parallel chain PAR
    isinter = False
    ...
    # Start the gateway of the parallel chain PAP
    isinter = False
...
async def test_tor_async(setting: TestSetting):
    ...
    The parallel chains use the PAP gateway (only responsible for synchronizing cross-chain transactions), and the parallel chains and the relay chain use the PAR gateway (only responsible for synchronizing block headers);
    向每个链发送的交易数量是固定的, 跨链交易占固定比例, 跨链交易的目的链标识是随机的;
    """
