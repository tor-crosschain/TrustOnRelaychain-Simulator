...
def build_relayers(
    ...
    if this_idx == 30000:
        ...
        # relay chain connects to parallel chain
    ...
    if ccmode == "NoR":
        ...
        ), f"inter pid is { relayer.dst.pid }"  # 中继链的标识为 30000
        relayers.append(relayer)
    elif ccmode == "ToR":
        src_connector = Connector(
            host=this_host, sender=f"proc/{this_idx}/{this_idx}/src"
        ...
        assert (
            relayer.dst.pid == 30000
        ), f"inter pid is { relayer.dst.pid }"  # relay chain's id is 30000
        relayers.append(relayer)
        lh = len(hosts)
        for i in range(lh):
            if this_host == hosts[i]:
