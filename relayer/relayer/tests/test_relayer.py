...
def test_listen():
    # start two chains environment
    threads, stop_api_events, bcs, mpools, api_servs, hosts, tasks = pre_test()
    set_pre(
        hosts, list(range(len(hosts))), ["Tendermint" for _ in range(len(hosts))], bcs
    )

    # send cross-chain transactions to the source chain
    # memo = {"type": "SRC-CTX", "dst": 1}
    # wait for the source chain to process
    txnum = 60
    ...
    # wait for the relayer to listen to all messages
...
def test_deal_header():
    ...
    while True:
        if mpools[0].empty():
            break
    assert mpools[0].empty()
    bcs[0].stop()  # source chain stop mining
    ...
    # wait for deal_header to synchronize all source chain headers to target
    time.sleep(5)
    assert relayer.queue_header.qsize() == 0

    # wait for the target chain to process
    while True:
        if mpools[1].empty():
            break
    assert mpools[1].empty()
    bcs[1].stop()  # target chain stop mining

    # check the total number of transactions on the target chain
    dtxa = 0
