...
class DstTool:
    def __init__(
        ...
        assert isinstance(connector, AsyncConnector)
...
class Dsts:
    ...
    def store_connector(self, connector: AsyncConnector) -> DstTool:
        assert isinstance(
            connector, AsyncConnector
...
class MultiplexAsyncRelayer(object):
    def __init__(
        self,
        src: AsyncConnector,
        # dst: AsyncConnector,
    ...
    def add_dst(self, dst: AsyncConnector):
        assert isinstance(dst, AsyncConnector)
        assert (
    ...
    async def filter_header(self, header: dict):
        ...
        if self.mode == Mode.MODEAOR:
            if self.src.isinter:
                # if the source is inter, then forward to each parachain
                dst_ids += self.dsts.all_pids()
            else:
                # if the source is not inter, then forward to the relay chain inter
                dst_ids += [30000]
    ...
    async def listen_block(self):
        ...
        """
        # only sync from the latest block header of src
        ...
        while not self.listen_event_stop.is_set():
            try:
                ...
                if str(e).startswith("response error: cannot find block"):
                    await asyncio.sleep(2)
                else:
                    raise Exception(str(e))
        print("listen stop!")

    async def deal_xtx(self, dstool: DstTool, inter: Optional[AsyncConnector] = None):
        """
        ...
        async def wait_header(height: int, synced_height: int) -> int:
            if self.mode in [Mode.MODENOR, Mode.MODEAOR]:
                ...
                while synced_inter_height < height:
                    ...
                    await asyncio.sleep(0.5)
                inter_height = await inter.query_header_cs_height(self.src.pid, height)
    ...
    async def deal_header(self, dstool: DstTool, inter: AsyncConnector = None):
        """
