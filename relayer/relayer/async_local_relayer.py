...
def mark_tx(tx: Transaction, time_key: str):
    ...
    # if len(ts[relayer_key]) == 0 and time_key == 'listened-x':
    #     print(tx.as_json())

    # if len(ts[relayer_key]) == 1 and ts[relayer_key][0][0] == time_key:
    #     print(tx.as_json())
...
class AsyncLocalRelayer(object):
    def __init__(
        self,
        src: AsyncLocalConnector,
        dst: AsyncLocalConnector,
        mode: str,
        inter: AsyncLocalConnector = None,
        rlytp: ToRRelayerType = None,
    ) -> None:
        assert isinstance(src, AsyncLocalConnector)
        assert isinstance(dst, AsyncLocalConnector)
        self.src = src
        self.dst = dst
        ...
        if mode == Mode.MODETOR:
            ...
            # if pap is true, then inter: Blockchain cannot be empty
            ...
            if self.rlytp == ToRRelayerType.PAP:
                ...
                assert isinstance(self.inter, AsyncLocalConnector)
            else:
        ...
        for tx in txs:
            if self.is_xtx(tx):
                if tx.status == TxStatus.SUCCESS:
                    tx = Transaction.from_json(tx.as_json())
                    mark_tx(tx, time_key="listened")
                    await self.queue_xtx.put(tx)
                    cnt += 1
                else:
                    print(f"tx: {tx.as_json()}")
        return cnt

    async def listen_block(self, src: AsyncLocalConnector, dst: AsyncLocalConnector):
        """
        ...
        if maxbn == 0:  # if dst has not received the block header from src
    ...
    @staticmethod
    def start_two_way(s2d: AsyncLocalRelayer, d2s: AsyncLocalRelayer):
